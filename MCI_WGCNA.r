...
## TODO make sure box and whisker has info on significance, yellow looks as small effect
## maybe do boxplots with standard errors instead
## make sure you know what is being plotted, is -.15 to .15 ME values? t-values?
## SG says seems strange that cases are more tight distribution than controls
## check out some scatterplots and stuff
...
## TODO let's make sure ctl is left and mci is right
g = ggplot(df_plot, aes(x = label, y = value, fill = label, group = interaction(color, Dx))) +
  ...
  labs(title = "Eigengene Differential Expression", subtitle = "Controls (left) vs Cases (right)")
print(g)
dev.off()
...
## TODO sva a couple SVs and account for them in the lm fit with eigengenes?
# mod = model.matrix(~ FACTOR_dx,phenos)
mod = model.matrix(~ FACTOR_dx,phenos)
svobj = sva(t(MEs), mod)     ## TODO insert this number into the graph maybe?
...
g = ggplot(df_plot, aes(x = label, y = value, fill = label, group = interaction(color, Dx))) +
  ...
  scale_x_discrete(breaks=df_plot$label, labels=df_plot$color) +
  scale_fill_manual(values = labels2colors(0:(length(unique(df_plot$color))-1))) +
  labs(title = paste("Eigengene Residuals after",svobj$n.sv,"SV(s) removed"), subtitle = "Controls (left) vs Cases (right)")
print(g)
dev.off()
...
##########TODO ensure that this can handle the mirnas and other stuff
...
cat("\nTODO: nice explanation of what this table is.")
table(grepl("REACTOME", unique(go_terms$PATHWAY)))


############## TODO module significance between phenos
############### remember to allow for more than 2 groups... just vs controls?
